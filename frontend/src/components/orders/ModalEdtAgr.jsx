...
const ModalOrdenServicio = ({ idSeleccionado, onClose, onSuccess }) => {
    ...
    const handleSave = async () => {
        if (!orden.id_usuario || !orden.id_moto || !orden.id_tecnico_asignado || !orden.id_estado_de_servicio) {
        ...
        try {
            if (idSeleccionado) {
                ...
                mostrarToast("¡Orden creada con éxito!", "success");
            }
            onSuccess();
            onClose();
        } catch (error) {
            console.error("Error al guardar orden:", error);
            const mensaje = error.response?.data?.message || "Error al procesar la orden de servicio";
