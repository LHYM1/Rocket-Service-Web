describe('Módulo de Administración - Gestión de Motos (CRUD)', () => {
  ...
  beforeEach(() => {
    ...
    cy.intercept('GET', '**/api/motos/listar*').as('getMotos');
    cy.intercept('GET', '**/api/clientes/listar*').as('getClientes');
    cy.intercept('POST', '**/api/motos/crear*').as('crearMoto');
    cy.intercept('PUT', '**/api/motos/modificar/*').as('actualizarMoto');
    cy.intercept('PUT', '**/api/motos/eliminar/*').as('desactivarMoto');
